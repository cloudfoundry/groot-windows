...
func (w *baseLayerWriter) Close() (err error) {
	...
	defer func() {
		...
		oc.SetSpanStatus(w.s, err)
	}()
	defer func() {
		fmt.Fprintf(os.Stderr, "MEOW: defer root.Close\n")
		w.root.Close()
		w.root = nil
	}()

	fmt.Fprintf(os.Stderr, "MEOW: calling baseLayerWriter.Close()\n")

	fmt.Fprintf(os.Stderr, "MEOW: closeCurrentFile\n")
	err = w.closeCurrentFile()
	if err != nil {
		return err
	}
	if w.err == nil {
		...
		fmt.Fprintf(os.Stderr, "MEOW: ProcessBaseLayer\n")
	...
	fmt.Fprintf(os.Stderr, "MEOW: DONE calling baseLayerWriter.Close()\n")
	return w.err
}
